...
class RecipeFilterSet(filters.FilterSet):
    tags = django_filters.CharFilter('tags__slug')
    is_favorited = django_filters.NumberFilter(method='is_favorited_filter', distinct=True)
    is_in_shopping_cart = django_filters.NumberFilter(method='is_in_shopping_cart_filter', distinct=True)
    class Meta:
        model = Recipe
        fields = (
            'is_favorited',
    ...
    def binary_validator(self, name, value):
        if value < 0 or value > 1:
            raise ValidationError(f'Параметр "{name}" может иметь значения "0" или "1".')
        return value
    ...
    def is_favorited_filter(self, queryset, name, value):
        """Фильтр Для поля is_favorited. Выводит только избранное или исключает избранное."""
        self.binary_validator(name=name, value=value)
        user = self.request.user
        if user.is_anonymous:
            return queryset
        favorite_recipes = Recipe.objects.filter(favorites__user=user)
        exclude_favorite_recipes = Recipe.objects.exclude(favorites__user=user)
